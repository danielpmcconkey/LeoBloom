...
let private setupReceivableAccounts (txn: NpgsqlTransaction) (prefix: string) =
    let revenueTypeId = InsertHelpers.insertAccountType txn $"{prefix}_rev" "credit"
    let assetTypeId = InsertHelpers.insertAccountType txn $"{prefix}_ast" "debit"
    let sourceAcctId = InsertHelpers.insertAccount txn $"{prefix}SR" $"{prefix}_revenue" revenueTypeId true
    let destAcctId = InsertHelpers.insertAccount txn $"{prefix}DS" $"{prefix}_asset" assetTypeId true
    (sourceAcctId, destAcctId)

/// Create account types and accounts for a payable: asset (source) + expense (dest).
/// Returns (sourceAccountId, destAccountId).
let private setupPayableAccounts (txn: NpgsqlTransaction) (prefix: string) =
    let assetTypeId = InsertHelpers.insertAccountType txn $"{prefix}_ast" "debit"
    let expenseTypeId = InsertHelpers.insertAccountType txn $"{prefix}_exp" "debit"
    let sourceAcctId = InsertHelpers.insertAccount txn $"{prefix}SR" $"{prefix}_asset" assetTypeId true
    let destAcctId = InsertHelpers.insertAccount txn $"{prefix}DS" $"{prefix}_expense" expenseTypeId true
    (sourceAcctId, destAcctId)
...
let private createConfirmedInstance
    ...
    let instanceId =
        InsertHelpers.insertObligationInstance txn agreementId instanceName true
    let transCmd : TransitionCommand =
        { instanceId = instanceId
          ...
          journalEntryId = None
          notes = None }
    match ObligationInstanceService.transition txn transCmd with
    | Ok _ -> instanceId
...
let private queryInstance (txn: NpgsqlTransaction) (instanceId: int) =
    use cmd = new NpgsqlCommand(
...
let ``posting a confirmed receivable instance creates correct journal entry and transitions to posted`` () =
    ...
    let (sourceAcctId, destAcctId) = setupReceivableAccounts txn prefix
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) true
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1200m (DateOnly(2091, 4, 15))

    let result = ObligationPostingService.postToLedger txn { instanceId = instanceId }
    match result with
    | Ok posted ->
        // Assert journal entry has 2 lines
        let lines = queryJournalEntryLines txn posted.journalEntryId
        ...
        let inst = queryInstance txn instanceId
...
let ``posting a confirmed payable instance creates correct journal entry and transitions to posted`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let (sourceAcctId, destAcctId) = setupPayableAccounts txn prefix
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) true
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "payable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 850m (DateOnly(2091, 4, 15))
...
let ``journal entry description matches agreement and instance names`` () =
    ...
    let agrId = insertAgreementWithAccounts txn agrName "receivable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId instName 1000m (DateOnly(2091, 4, 15))

    let result = ObligationPostingService.postToLedger txn { instanceId = instanceId }
    match result with
    | Ok posted ->
        let je = queryJournalEntry txn posted.journalEntryId
...
let ``journal entry has obligation source and reference`` () =
    ...
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) true
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1000m (DateOnly(2091, 4, 15))
    ...
    | Ok posted ->
        ...
        // Assert reference
        let refs = queryJournalEntryReferences txn posted.journalEntryId
...
let ``journal entry entry_date equals the instance confirmed_date`` () =
    ...
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 500m confirmedDate
...
let ``posting an instance not in confirmed status returns error`` () =
    ...
    let instanceId =
        InsertHelpers.insertObligationInstance txn agrId $"{prefix}_inst" true
...
let ``posting an instance with no amount returns error`` () =
    ...
    let instanceId =
        InsertHelpers.insertObligationInstanceFull
            txn agrId $"{prefix}_inst" "confirmed"
    ...
    use updCmd = new NpgsqlCommand(
        "UPDATE ops.obligation_instance SET confirmed_date = @cd WHERE id = @id", txn.Connection)
...
let ``posting when agreement has no source_account_id returns error`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let (_, destAcctId) = setupReceivableAccounts txn prefix
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) true
    // No source account on agreement
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" None (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1000m (DateOnly(2091, 4, 15))
...
let ``posting when agreement has no dest_account_id returns error`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let (sourceAcctId, _) = setupReceivableAccounts txn prefix
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) true
    // No dest account on agreement
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" (Some sourceAcctId) None
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1000m (DateOnly(2091, 4, 15))
...
let ``posting when no fiscal period covers confirmed_date returns error`` () =
    ...
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1000m (DateOnly(2091, 7, 15))
...
let ``posting when fiscal period is closed returns error`` () =
    ...
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 4, 1)) (DateOnly(2091, 4, 30)) false
    let agrId = insertAgreementWithAccounts txn $"{prefix}_agr" "receivable" (Some sourceAcctId) (Some destAcctId)
    let instanceId = createConfirmedInstance txn agrId $"{prefix}_inst" 1000m (DateOnly(2091, 4, 15))
...
let ``posting an already-posted instance is rejected with no duplicate journal entry`` () =
    ...
    let firstResult = ObligationPostingService.postToLedger txn { instanceId = instanceId }
    ...
    | Ok posted ->
        ...
        let secondResult = ObligationPostingService.postToLedger txn { instanceId = instanceId }
...
let ``retry after partial failure skips duplicate journal entry`` () =
    ...
    let preExistingJeId =
        use jeCmd = new NpgsqlCommand(
            "INSERT INTO ledger.journal_entry (entry_date, description, source, fiscal_period_id) \
             VALUES (@d, @desc, 'obligation', @fp) RETURNING id", txn.Connection)
    ...
    use refCmd = new NpgsqlCommand(
        "INSERT INTO ledger.journal_entry_reference (journal_entry_id, reference_type, reference_value) \
         VALUES (@je, 'obligation', @rv)", txn.Connection)
...
let ``findByDate returns correct period for a date within range`` () =
    ...
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" startDate endDate true
...
let ``findByDate returns None for a date outside all periods`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let fpId = InsertHelpers.insertFiscalPeriod txn $"{prefix}FP" (DateOnly(2091, 5, 1)) (DateOnly(2091, 5, 31)) true
