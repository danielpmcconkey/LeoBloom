...
let private postMultiLineEntry (txn: NpgsqlTransaction) fpId (entryDate: DateOnly) (desc: string) (lines: PostLineCommand list) =
    let cmd =
        { entryDate = entryDate
          description = desc
          source = Some "manual"
...
let ``report groups accounts by type with correct subtotals`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let assetAcct1 = InsertHelpers.insertAccount txn (prefix + "A1") "Asset1" assetTypeId true
    let assetAcct2 = InsertHelpers.insertAccount txn (prefix + "A2") "Asset2" assetTypeId true
    let revAcct = InsertHelpers.insertAccount txn (prefix + "RV") "Revenue" revenueTypeId true
    ...
    postMultiLineEntry txn fpId (DateOnly(2026, 3, 10)) "Income"
...
let ``voided entries excluded from trial balance`` () =
    ...
    let revAcct = InsertHelpers.insertAccount txn (prefix + "RV") "Revenue" revenueTypeId true
    let fpId = InsertHelpers.insertFiscalPeriod txn (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
    let _entryId1 = postEntry txn assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Good entry" 500m
    let entryId2 = postEntry txn assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Bad entry" 200m
...
let ``closed period trial balance still works`` () =
    ...
    postEntry txn assetAcct revAcct fpId (DateOnly(2025, 12, 15)) "December entry" 800m |> ignore
    // Close the period
