...
let ``Running tests creates a log file`` () =
    // Log.initialize was called earlier in the test run. Just verify a log file exists.
...
let ``Querying account balance by id emits Info-level log entry`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let assetAt = InsertHelpers.insertAccountType txn (prefix + "_as") "debit"
    let acctId = InsertHelpers.insertAccount txn (prefix + "AS") "Asset" assetAt true

    let _result = AccountBalanceService.getBalanceById txn acctId (DateOnly(2026, 3, 31))
...
let ``Querying account balance by code emits Info-level log entry`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let assetAt = InsertHelpers.insertAccountType txn (prefix + "_as") "debit"
    let _acctId = InsertHelpers.insertAccount txn (prefix + "AS") "Asset" assetAt true
    let acctCode = prefix + "AS"

    let _result = AccountBalanceService.getBalanceByCode txn acctCode (DateOnly(2026, 3, 31))
