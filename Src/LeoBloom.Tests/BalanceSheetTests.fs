...
let ``balanced books produce isBalanced true`` () =
    ...
    try
        ...
        let liabAcct = InsertHelpers.insertAccount conn tracker (prefix + "LI") "Liability" liabilityTypeId true
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        ...
        postEntry conn tracker assetAcct eqAcct fpId (DateOnly(2026, 3, 5)) "Owner investment" 5000m |> ignore
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Service income" 1000m |> ignore
        postEntry conn tracker expAcct assetAcct fpId (DateOnly(2026, 3, 20)) "Office rent" 400m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(2026, 3, 31))
        match result with
        | Ok report ->
...
let ``assets equal liabilities plus total equity`` () =
    ...
    try
        ...
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct eqAcct fpId (DateOnly(2026, 3, 5)) "Owner investment" 5000m |> ignore
        postEntry conn tracker assetAcct liabAcct fpId (DateOnly(2026, 3, 10)) "Took a loan" 2000m |> ignore
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Earned revenue" 1000m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(2026, 3, 31))
        match result with
        | Ok report ->
...
let ``positive retained earnings when revenue exceeds expenses`` () =
    ...
    try
        ...
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(1950, 3, 1)) (DateOnly(1950, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(1950, 3, 10)) "Revenue" 3000m |> ignore
        postEntry conn tracker expAcct assetAcct fpId (DateOnly(1950, 3, 20)) "Expense" 1000m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(1950, 3, 31))
        match result with
        | Ok report ->
            let assetLine = report.assets.lines |> List.find (fun l -> l.accountId = assetAcct)
...
let ``negative retained earnings when expenses exceed revenue`` () =
    ...
    try
        ...
        let expAcct = InsertHelpers.insertAccount conn tracker (prefix + "EX") "Expense" expenseTypeId true
        // Use far-past dates so parallel tests' revenue/expense entries don't pollute retainedEarnings
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(1951, 3, 1)) (DateOnly(1951, 3, 31)) true
        postEntry conn tracker assetAcct liabAcct fpId (DateOnly(1951, 3, 5)) "Borrowed to fund operations" 2000m |> ignore
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(1951, 3, 10)) "Small revenue" 200m |> ignore
        postEntry conn tracker expAcct assetAcct fpId (DateOnly(1951, 3, 20)) "Large expense" 800m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(1951, 3, 31))
        match result with
        | Ok report ->
            let assetLine = report.assets.lines |> List.find (fun l -> l.accountId = assetAcct)
...
let ``retained earnings zero when no revenue or expense activity`` () =
    ...
    try
        ...
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        // Use far-past dates so parallel tests' revenue/expense entries don't pollute retainedEarnings
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(1952, 3, 1)) (DateOnly(1952, 3, 31)) true
        postEntry conn tracker assetAcct eqAcct fpId (DateOnly(1952, 3, 10)) "Owner investment only" 5000m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(1952, 3, 31))
        match result with
        | Ok report ->
            let assetLine = report.assets.lines |> List.find (fun l -> l.accountId = assetAcct)
...
let ``before any entries all zeros and balanced`` () =
    ...
    let result = BalanceSheetService.getAsOfDate (DateOnly(1900, 1, 1))
    match result with
    | Ok report ->
...
let ``voided entries excluded from balance sheet`` () =
    ...
    try
        ...
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct eqAcct fpId (DateOnly(2026, 3, 10)) "Good investment" 5000m |> ignore
        let badEntryId = postEntry conn tracker assetAcct eqAcct fpId (DateOnly(2026, 3, 15)) "Bad investment" 2000m
...
let ``entries across multiple fiscal periods all contribute`` () =
    ...
    try
        ...
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revenueTypeId true
        let fp1 = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "P1") (DateOnly(2026, 1, 1)) (DateOnly(2026, 1, 31)) true
        let fp2 = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "P2") (DateOnly(2026, 2, 1)) (DateOnly(2026, 2, 28)) true
        let fp3 = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "P3") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct eqAcct fp1 (DateOnly(2026, 1, 15)) "January investment" 3000m |> ignore
        postEntry conn tracker assetAcct revAcct fp2 (DateOnly(2026, 2, 15)) "February income" 1000m |> ignore
        postEntry conn tracker assetAcct revAcct fp3 (DateOnly(2026, 3, 15)) "March income" 500m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(2026, 3, 31))
        match result with
        | Ok report ->
...
let ``multiple accounts per section accumulate correctly`` () =
    ...
    try
        let prefix = TestData.uniquePrefix()
        let asset1 = InsertHelpers.insertAccount conn tracker (prefix + "A1") "Asset1" assetTypeId true
        let asset2 = InsertHelpers.insertAccount conn tracker (prefix + "A2") "Asset2" assetTypeId true
        let liab1 = InsertHelpers.insertAccount conn tracker (prefix + "L1") "Liability1" liabilityTypeId true
        let liab2 = InsertHelpers.insertAccount conn tracker (prefix + "L2") "Liability2" liabilityTypeId true
        let eq1 = InsertHelpers.insertAccount conn tracker (prefix + "Q1") "Equity1" equityTypeId true
        let eq2 = InsertHelpers.insertAccount conn tracker (prefix + "Q2") "Equity2" equityTypeId true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker asset1 eq1 fpId (DateOnly(2026, 3, 5)) "Owner investment into checking" 5000m |> ignore
        postEntry conn tracker asset2 liab1 fpId (DateOnly(2026, 3, 10)) "Equipment purchase on credit" 2000m |> ignore
        postEntry conn tracker asset1 eq2 fpId (DateOnly(2026, 3, 15)) "Partner investment" 3000m |> ignore
        postEntry conn tracker asset1 liab2 fpId (DateOnly(2026, 3, 20)) "Additional loan" 1000m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(2026, 3, 31))
        match result with
        | Ok report ->
...
let ``account with activity netting to zero still appears with balance zero`` () =
    ...
    try
        ...
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker asset1 eqAcct fpId (DateOnly(2026, 3, 5)) "Investment" 5000m |> ignore
        postEntry conn tracker asset2 asset1 fpId (DateOnly(2026, 3, 10)) "Transfer to savings" 1000m |> ignore
        postEntry conn tracker asset1 asset2 fpId (DateOnly(2026, 3, 15)) "Transfer back" 1000m |> ignore
        let result = BalanceSheetService.getAsOfDate (DateOnly(2026, 3, 31))
        match result with
        | Ok report ->
...
let ``inactive accounts with cumulative balances still appear`` () =
    ...
    try
        ...
        let eqAcct = InsertHelpers.insertAccount conn tracker (prefix + "EQ") "Equity" equityTypeId true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct eqAcct fpId (DateOnly(2026, 3, 15)) "Before deactivation" 5000m |> ignore
...
let ``getAsOfDate returns Result of BalanceSheetReport or string`` () =
    let result : Result<BalanceSheetReport, string> = BalanceSheetService.getAsOfDate (DateOnly(1900, 1, 1))
    match result with
    | Ok report ->
