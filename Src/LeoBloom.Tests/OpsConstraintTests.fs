...
let ``obligation_agreement amount is nullable`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc017_agreement"
    Assert.True(oaId > 0)
...
let ``obligation_instance requires a name`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc020_agreement"
    let ex = ConstraintAssert.tryExec conn
...
let ``obligation_instance requires an expected_date`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc023_agreement"
    let ex = ConstraintAssert.tryExec conn
...
let ``obligation_instance journal_entry_id must reference a valid journal_entry`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc024_agreement"
    let ex = ConstraintAssert.tryExec conn
...
let ``obligation_instance journal_entry_id is nullable`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc025_agreement"
    use cmd = new NpgsqlCommand(
        "INSERT INTO ops.obligation_instance (obligation_agreement_id, name, status, expected_date, journal_entry_id) VALUES (@oa, 'Test', 'expected', '2026-04-01', NULL)", txn.Connection)
    cmd.Transaction <- txn
    cmd.Parameters.AddWithValue("@oa", oaId) |> ignore
...
let private insertTwoAccounts (txn: NpgsqlTransaction) (prefix: string) =
...
let ``transfer requires a from_account_id`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (_, toId) = insertTwoAccounts txn "osc026"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer from_account_id must reference a valid ledger account`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (_, toId) = insertTwoAccounts txn "osc027"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer requires a to_account_id`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, _) = insertTwoAccounts txn "osc028"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer to_account_id must reference a valid ledger account`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, _) = insertTwoAccounts txn "osc029"
    let ex = ConstraintAssert.tryExec conn
                "INSERT INTO ops.transfer (from_account_id, to_account_id, amount, status, initiated_date) VALUES (@from_, @to_, 100.00, 'initiated', '2026-04-01')"
                (fun cmd ->
...
let ``transfer requires an amount`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, toId) = insertTwoAccounts txn "osc030"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer requires a status`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, toId) = insertTwoAccounts txn "osc031"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer requires an initiated_date`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, toId) = insertTwoAccounts txn "osc032"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer journal_entry_id must reference a valid journal_entry`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let (fromId, toId) = insertTwoAccounts txn "osc033"
    let ex = ConstraintAssert.tryExec conn
...
let ``transfer journal_entry_id is nullable`` () =
    ...
    let (fromId, toId) = insertTwoAccounts txn prefix
    use cmd = new NpgsqlCommand(
        "INSERT INTO ops.transfer (from_account_id, to_account_id, amount, status, initiated_date, journal_entry_id) VALUES (@from_, @to_, 100.00, 'initiated', '2026-04-01', NULL)", txn.Connection)
    cmd.Transaction <- txn
    cmd.Parameters.AddWithValue("@from_", fromId) |> ignore
    cmd.Parameters.AddWithValue("@to_", toId) |> ignore
    cmd.ExecuteNonQuery() |> ignore
...
let ``invoice requires a tenant`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let fpId = InsertHelpers.insertFiscalPeriod txn "o035fp" (System.DateOnly(2099, 1, 1)) (System.DateOnly(2099, 1, 31)) true
    let ex = ConstraintAssert.tryExec conn
...
let ``invoice requires rent_amount`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let fpId = InsertHelpers.insertFiscalPeriod txn "o038fp" (System.DateOnly(2099, 1, 1)) (System.DateOnly(2099, 1, 31)) true
    let ex = ConstraintAssert.tryExec conn
...
let ``invoice requires utility_share`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let fpId = InsertHelpers.insertFiscalPeriod txn "o039fp" (System.DateOnly(2099, 1, 1)) (System.DateOnly(2099, 1, 31)) true
    let ex = ConstraintAssert.tryExec conn
...
let ``invoice requires total_amount`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let fpId = InsertHelpers.insertFiscalPeriod txn "o040fp" (System.DateOnly(2099, 1, 1)) (System.DateOnly(2099, 1, 31)) true
    let ex = ConstraintAssert.tryExec conn
...
let ``obligation_instance requires a status`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let oaId = InsertHelpers.insertObligationAgreement txn "osc044_agreement"
    let ex = ConstraintAssert.tryExec conn
