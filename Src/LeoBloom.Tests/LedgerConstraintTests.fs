...
let private assertNotNull = assertSqlState "23502"
let private assertUnique = assertSqlState "23505"
let private assertFk = assertSqlState "23503"
...
let ``account_type requires a name`` () =
    ...
    try
        let ex = tryInsert conn "INSERT INTO ledger.account_type (name, normal_balance) VALUES (NULL, 'debit')"
        assertNotNull ex "Expected NOT NULL violation"
...
let ``account_type name must be unique`` () =
    ...
    try
        ...
        assertUnique ex "Expected UNIQUE violation"
    finally TestCleanup.deleteAll tracker
...
let ``account_type requires a normal_balance`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.account_type (name, normal_balance) VALUES ('test_type', NULL)"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``account requires an account_type_id`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.account (code, name, account_type_id) VALUES ('ZZZZ', 'Test', NULL)"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``account account_type_id must reference a valid account_type`` () =
    ...
    try
        ...
        assertFk ex "Expected FK violation"
    finally TestCleanup.deleteAll tracker
...
let ``fiscal_period requires a period_key`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.fiscal_period (period_key, start_date, end_date) VALUES (NULL, '2099-01-01', '2099-01-31')"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``fiscal_period requires a start_date`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.fiscal_period (period_key, start_date, end_date) VALUES ('2099-01', NULL, '2099-01-31')"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``fiscal_period requires an end_date`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.fiscal_period (period_key, start_date, end_date) VALUES ('2099-01', '2099-01-01', NULL)"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``journal_entry requires a fiscal_period_id`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.journal_entry (entry_date, description, fiscal_period_id) VALUES ('2026-01-01', 'Test', NULL)"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
...
let ``journal_entry_reference requires a journal_entry_id`` () =
    ...
    let tracker = TestCleanup.create conn
    try
        let ex = tryInsert conn "INSERT INTO ledger.journal_entry_reference (journal_entry_id, reference_type, reference_value) VALUES (NULL, 'invoice', 'INV-001')"
        assertNotNull ex "Expected NOT NULL violation"
    finally TestCleanup.deleteAll tracker
