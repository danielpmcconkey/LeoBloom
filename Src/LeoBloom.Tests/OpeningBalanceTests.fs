...
let ``opening balances for asset and liability produce balanced entry`` () =
    ...
    let cashAcct = InsertHelpers.insertAccount txn (prefix + "CA") "Cash" assetTypeId true
    let mortgageAcct = InsertHelpers.insertAccount txn (prefix + "MO") "Mortgage" liabilityTypeId true
    let equityAcct = InsertHelpers.insertAccount txn (prefix + "EQ") "Equity" equityTypeId true
    let fpId = InsertHelpers.insertFiscalPeriod txn (prefix + "FP") (DateOnly(2026, 1, 1)) (DateOnly(2026, 1, 31)) true
    let cmd : PostOpeningBalancesCommand =
        { entryDate = DateOnly(2026, 1, 1)
          ...
          description = None }
    match OpeningBalanceService.post txn cmd with
    | Ok posted ->
...
let ``posted entry is retrievable and has correct line count`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let cashAcct = InsertHelpers.insertAccount txn (prefix + "CA") "Cash" assetTypeId true
    let brokerAcct = InsertHelpers.insertAccount txn (prefix + "BR") "Brokerage" assetTypeId true
    let mortgageAcct = InsertHelpers.insertAccount txn (prefix + "MO") "Mortgage" liabilityTypeId true
    let equityAcct = InsertHelpers.insertAccount txn (prefix + "EQ") "Equity" equityTypeId true
    let fpId = InsertHelpers.insertFiscalPeriod txn (prefix + "FP") (DateOnly(2026, 1, 1)) (DateOnly(2026, 1, 31)) true
    let cmd : PostOpeningBalancesCommand =
