...
let ``single overdue instance is transitioned`` () =
    ...
    InsertHelpers.insertObligationInstanceFull
        txn agreementId $"{prefix}_inst" "expected"
        (DateOnly(2026, 3, 15)) None None true |> ignore

    let result = ObligationInstanceService.detectOverdue txn (DateOnly(2026, 4, 1))
...
let ``multiple overdue instances are all transitioned`` () =
    ...
    for month in [ 1; 2; 3 ] do
        InsertHelpers.insertObligationInstanceFull
            txn agreementId $"{prefix}_m{month}" "expected"
...
let ``in_flight instances are not flagged as overdue`` () =
    ...
    let agreementId =
        InsertHelpers.insertObligationAgreementForSpawn
            txn $"{prefix}_agr" "receivable" "monthly" (Some 1) None true
    InsertHelpers.insertObligationInstanceFull
        txn agreementId $"{prefix}_inst" "in_flight"
        (DateOnly(2026, 3, 15)) None None true |> ignore
...
let ``already overdue instances are not re-transitioned`` () =
    ...
    let agreementId =
        InsertHelpers.insertObligationAgreementForSpawn
            txn $"{prefix}_agr" "receivable" "monthly" (Some 1) None true
    InsertHelpers.insertObligationInstanceFull
        txn agreementId $"{prefix}_inst" "overdue"
        (DateOnly(2026, 3, 15)) None None true |> ignore
...
let ``confirmed instances are not flagged as overdue`` () =
    ...
    let agreementId =
        InsertHelpers.insertObligationAgreementForSpawn
            txn $"{prefix}_agr" "receivable" "monthly" (Some 1) None true
    InsertHelpers.insertObligationInstanceFull
        txn agreementId $"{prefix}_inst" "confirmed"
...
let ``running detection twice produces same result`` () =
    ...
    let result1 = ObligationInstanceService.detectOverdue txn (DateOnly(2026, 4, 1))
    Assert.Equal(1, result1.transitioned)

    let result2 = ObligationInstanceService.detectOverdue txn (DateOnly(2026, 4, 1))
