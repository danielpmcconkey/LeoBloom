...
let private setupActiveFiscalPeriod (txn: Npgsql.NpgsqlTransaction) (prefix: string) =
    InsertHelpers.insertFiscalPeriod txn (TestData.periodKey prefix) (DateOnly(2099, 4, 1)) (DateOnly(2099, 4, 30)) true

/// Create a closed fiscal period and return its id.
let private setupClosedFiscalPeriod (txn: Npgsql.NpgsqlTransaction) (prefix: string) =
    InsertHelpers.insertFiscalPeriod txn (TestData.periodKey prefix) (DateOnly(2099, 5, 1)) (DateOnly(2099, 5, 31)) false
...
/// Record an invoice through the service, assert success, return Invoice.
let private recordInv (txn: Npgsql.NpgsqlTransaction) (cmd: RecordInvoiceCommand) : Invoice =
    match InvoiceService.recordInvoice txn cmd with
    | Ok inv -> inv
...
let ``Recording a valid invoice persists it and returns a complete record`` () =
    ...
    let fpId = setupActiveFiscalPeriod txn prefix
...
let ``Recording an invoice for a closed fiscal period succeeds`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let fpId = setupClosedFiscalPeriod txn prefix

    let cmd = { defaultCmd $"{prefix}_Jeffrey" fpId with totalAmount = 1285.50m }
    let result = InvoiceService.recordInvoice txn cmd

    match result with
    | Ok inv -> Assert.True(inv.id > 0)
    | Error errs -> Assert.Fail(sprintf "Expected Ok: %A" errs)
...
let ``Recording with empty tenant is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for empty tenant")
    | Error errs ->
...
let ``Recording with tenant exceeding 50 characters is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for long tenant")
    | Error errs ->
        Assert.True(errs |> List.exists (fun e -> e.ToLowerInvariant().Contains("tenant")),
                    sprintf "Expected error containing 'tenant': %A" errs)
...
let ``Recording with negative rentAmount is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for negative rentAmount")
    | Error errs ->
...
let ``Recording with negative utilityShare is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for negative utilityShare")
    | Error errs ->
...
let ``Recording with negative totalAmount is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for negative totalAmount")
    | Error errs ->
...
let ``Recording with rentAmount having more than 2 decimal places is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for >2 decimal places")
    | Error errs ->
...
let ``Recording with total not equal to rent plus utility is rejected`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for mismatched total")
    | Error errs ->
...
let ``Recording with multiple validation errors collects all errors`` () =
    ...
    match result with
    | Ok _ -> Assert.Fail("Expected Error for multiple validation failures")
    | Error errs ->
...
let ``Recording a duplicate tenant and fiscal period is rejected`` () =
    ...
    let _firstInv = recordInv txn (defaultCmd tenant fpId)

    // Attempt duplicate
    let result = InvoiceService.recordInvoice txn (defaultCmd tenant fpId)

    match result with
    | Ok _ -> Assert.Fail("Expected Error for duplicate invoice")
    | Error errs ->
...
let ``Showing an invoice by ID returns the full record`` () =
    ...
    let fpId = setupActiveFiscalPeriod txn prefix
    let tenant = $"{prefix}_Jeffrey"
    let inv = recordInv txn (defaultCmd tenant fpId)

    let result = InvoiceService.showInvoice txn inv.id
...
let ``Showing a nonexistent invoice returns an error`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let result = InvoiceService.showInvoice txn 999999
...
let ``Listing invoices with no filter returns all active invoices`` () =
    ...
    let _invJ = recordInv txn (defaultCmd tenantJ fpId)
    let _invA = recordInv txn
                    { defaultCmd tenantA fpId with
                        rentAmount = 1000.00m
                        utilityShare = 50.00m
                        totalAmount = 1050.00m }

    let results = InvoiceService.listInvoices txn { tenant = None; fiscalPeriodId = None }
...
let ``Listing invoices filtered by tenant returns only that tenant`` () =
    ...
    let results = InvoiceService.listInvoices txn { tenant = Some tenantJ; fiscalPeriodId = None }
...
let ``Listing invoices filtered by fiscal period returns only that period`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let fp1 = InsertHelpers.insertFiscalPeriod txn $"{prefix}F1" (DateOnly(2099, 1, 1)) (DateOnly(2099, 1, 31)) true
    let fp2 = InsertHelpers.insertFiscalPeriod txn $"{prefix}F2" (DateOnly(2099, 2, 1)) (DateOnly(2099, 2, 28)) true
    let tenant = $"{prefix}_Jeffrey"

    let _inv1 = recordInv txn (defaultCmd tenant fp1)
    let _inv2 = recordInv txn
                    { defaultCmd tenant fp2 with
                        rentAmount = 1200.00m
                        utilityShare = 85.50m
                        totalAmount = 1285.50m }

    let results = InvoiceService.listInvoices txn { tenant = None; fiscalPeriodId = Some fp1 }
...
let ``Listing invoices filtered by both tenant and fiscal period`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let fp1 = InsertHelpers.insertFiscalPeriod txn $"{prefix}F1" (DateOnly(2099, 3, 1)) (DateOnly(2099, 3, 31)) true
    let fp2 = InsertHelpers.insertFiscalPeriod txn $"{prefix}F2" (DateOnly(2099, 4, 1)) (DateOnly(2099, 4, 30)) true
    let tenantJ = $"{prefix}_Jeffrey"
    let tenantA = $"{prefix}_Adam"

    let _invJF1 = recordInv txn (defaultCmd tenantJ fp1)
    let _invAF1 = recordInv txn
                    { defaultCmd tenantA fp1 with
                        rentAmount = 1000.00m
                        utilityShare = 50.00m
                        totalAmount = 1050.00m }
    let _invJF2 = recordInv txn
                    { defaultCmd tenantJ fp2 with
                        rentAmount = 1200.00m
                        utilityShare = 85.50m
                        totalAmount = 1285.50m }

    let results = InvoiceService.listInvoices txn { tenant = Some tenantJ; fiscalPeriodId = Some fp1 }
...
let ``Listing invoices returns empty when none match`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let results = InvoiceService.listInvoices txn { tenant = Some "Nobody"; fiscalPeriodId = None }
