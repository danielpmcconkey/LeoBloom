...
let ``normal-debit account balance after single entry`` () =
    ...
    try
        ...
        let assetAcct = InsertHelpers.insertAccount conn tracker (prefix + "AS") "Asset" assetAt true
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Single debit" 1000m |> ignore
        let result = AccountBalanceService.getBalanceById assetAcct (DateOnly(2026, 3, 31))
        match result with
        | Ok bal ->
...
let ``normal-credit account balance after single entry`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Single credit" 1000m |> ignore
        let result = AccountBalanceService.getBalanceById revAcct (DateOnly(2026, 3, 31))
        match result with
        | Ok bal ->
            Assert.Equal(1000m, bal.balance)
...
let ``balance accumulates across multiple entries`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Entry 1" 500m |> ignore
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 20)) "Entry 2" 300m |> ignore
        let result = AccountBalanceService.getBalanceById assetAcct (DateOnly(2026, 3, 31))
        match result with
...
let ``mixed debits and credits net correctly`` () =
    ...
    try
        ...
        let assetAcct = InsertHelpers.insertAccount conn tracker (prefix + "AS") "Asset" assetAt true
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let expAcct = InsertHelpers.insertAccount conn tracker (prefix + "EX") "Expense" expAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        // First entry: debit asset 1000, credit revenue 1000
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Debit entry" 1000m |> ignore
        // Second entry: debit expense 400, credit asset 400
        postEntry conn tracker expAcct assetAcct fpId (DateOnly(2026, 3, 20)) "Credit entry" 400m |> ignore
        let result = AccountBalanceService.getBalanceById assetAcct (DateOnly(2026, 3, 31))
        match result with
...
let ``voided entry excluded from balance`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        let _entryId1 = postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Keep entry" 500m
        let entryId2 = postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 20)) "Void entry" 200m
...
let ``entry after as_of_date excluded`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 10)) "Before cutoff" 500m |> ignore
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 25)) "After cutoff" 300m |> ignore
        let result = AccountBalanceService.getBalanceById assetAcct (DateOnly(2026, 3, 15))
        match result with
        | Ok bal -> Assert.Equal(500m, bal.balance)
        | Error err -> Assert.Fail(sprintf "Expected Ok: %s" err)
    finally TestCleanup.deleteAll tracker
...
let ``inactive account balance is calculated`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Before deactivate" 500m |> ignore
        use cmd = new NpgsqlCommand("UPDATE ledger.account SET is_active = false WHERE id = @id", conn)
...
let ``nonexistent account ID returns error`` () =
    let result = AccountBalanceService.getBalanceById 999999 (DateOnly(2026, 3, 31))
    match result with
...
let ``nonexistent account code returns error`` () =
    let result = AccountBalanceService.getBalanceByCode "ZZZZ" (DateOnly(2026, 3, 31))
    match result with
...
let ``lookup by account code matches lookup by ID`` () =
    ...
    try
        ...
        let revAcct = InsertHelpers.insertAccount conn tracker (prefix + "RV") "Revenue" revAt true
        let fpId = InsertHelpers.insertFiscalPeriod conn tracker (prefix + "FP") (DateOnly(2026, 3, 1)) (DateOnly(2026, 3, 31)) true
        postEntry conn tracker assetAcct revAcct fpId (DateOnly(2026, 3, 15)) "Code lookup" 750m |> ignore
        let result = AccountBalanceService.getBalanceByCode (prefix + "AS") (DateOnly(2026, 3, 31))
        match result with
        | Ok bal ->
