...
let ``create agreement with all fields provided`` () =
    ...
    let srcAcct = InsertHelpers.insertAccount txn (prefix + "S1") "Source" atId true
    let dstAcct = InsertHelpers.insertAccount txn (prefix + "D1") "Dest" atId true
    let cmd =
    ...
    | Ok agreement ->
        ...
        let persisted = ObligationAgreementService.getById txn agreement.id
...
let ``create with inactive source account is rejected`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let atId = InsertHelpers.insertAccountType txn (prefix + "_at") "debit"
    let inactiveAcct = InsertHelpers.insertAccount txn (prefix + "IA") "Inactive" atId false
...
let ``create with source_account_id equal to dest_account_id is rejected`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let atId = InsertHelpers.insertAccountType txn (prefix + "_at") "debit"
    let acctId = InsertHelpers.insertAccount txn (prefix + "AC") "Shared" atId true
...
let ``create with inactive dest account is rejected`` () =
    ...
    let prefix = TestData.uniquePrefix()
    let atId = InsertHelpers.insertAccountType txn (prefix + "_at") "debit"
    let inactiveDest = InsertHelpers.insertAccount txn (prefix + "ID") "Inactive Dest" atId false
...
let ``get agreement by ID returns the agreement`` () =
    ...
    let created =
        match ObligationAgreementService.create txn cmd with
        | Ok a -> a
        | Error errs -> failwithf "Setup failed: %A" errs
    let result = ObligationAgreementService.getById txn created.id
    match result with
    | Some found ->
...
let ``get agreement by nonexistent ID returns none`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let result = ObligationAgreementService.getById txn 999999
...
let ``list with default filter returns only active agreements`` () =
    ...
    let activeId = InsertHelpers.insertObligationAgreementFull txn activeName "receivable" "monthly" true
    let inactiveId = InsertHelpers.insertObligationAgreementFull txn inactiveName "receivable" "monthly" false
    ignore inactiveId
    let results = ObligationAgreementService.list txn defaultFilter
...
let ``list filtered by obligation type`` () =
    ...
    let _recvId = InsertHelpers.insertObligationAgreementFull txn receivableName "receivable" "monthly" true
    let payId = InsertHelpers.insertObligationAgreementFull txn payableName "payable" "monthly" true
...
let ``list filtered by cadence`` () =
    ...
    let monthlyId = InsertHelpers.insertObligationAgreementFull txn monthlyName "receivable" "monthly" true
    let _annualId = InsertHelpers.insertObligationAgreementFull txn annualName "receivable" "annual" true
...
let ``list returns empty when no agreements match`` () =
    ...
    use txn = conn.BeginTransaction()
    let prefix = TestData.uniquePrefix()
    let results = ObligationAgreementService.list txn { isActive = None; obligationType = None; cadence = None }
...
let ``update agreement with all fields`` () =
    ...
    let created =
        match ObligationAgreementService.create txn createCmd with
        | Ok a -> a
    ...
    let updateCmd =
        { defaultUpdateCmd created.id prefix with
            ...
            amount = Some 200.00m }
    let result = ObligationAgreementService.update txn updateCmd
    match result with
    | Ok updated ->
        ...
        Assert.True(updated.modifiedAt >= created.createdAt,
                    "modified_at should be no earlier than created_at")
        // REM-013: Verify persistence via independent read
        let persisted = ObligationAgreementService.getById txn updated.id
        match persisted with
        | Some p ->
...
let ``reactivate a previously deactivated agreement`` () =
    ...
    let agreementId = InsertHelpers.insertObligationAgreementFull txn dormantName "receivable" "monthly" false
    let updateCmd =
...
let ``deactivate an active agreement`` () =
    ...
    let created =
        ...
        | Ok a -> a
        | Error errs -> failwithf "Setup create failed: %A" errs
    let result = ObligationAgreementService.deactivate txn created.id
    match result with
    | Ok deactivated ->
        Assert.False(deactivated.isActive, "Expected isActive = false after deactivation")
        // REM-013: Verify persistence via independent read
        let persisted = ObligationAgreementService.getById txn deactivated.id
        match persisted with
...
let ``deactivate a nonexistent agreement is rejected`` () =
    use conn = DataSource.openConnection()
    use txn = conn.BeginTransaction()
    let result = ObligationAgreementService.deactivate txn 999999
    match result with
    | Ok _ -> Assert.Fail("Expected Error for nonexistent agreement")
    | Error errs ->
        Assert.True(errs |> List.exists (fun e -> e.ToLowerInvariant().Contains("does not exist")),
...
let ``deactivate blocked by active obligation instances`` () =
    ...
    let agreementId = InsertHelpers.insertObligationAgreement txn agreementName
    let _instanceId = InsertHelpers.insertObligationInstance txn agreementId $"{prefix}_inst" true
    let result = ObligationAgreementService.deactivate txn agreementId
    match result with
