...
module SubtreePLRepository =
    ...
    let getSubtreeActivityByPeriodAsOfClose (txn: NpgsqlTransaction) (rootAccountCode: string) (fiscalPeriodId: int) (closedAt: DateTimeOffset) : (string * IncomeStatementLine) list =
        use cmd = new NpgsqlCommand(
            "WITH RECURSIVE subtree AS (
                SELECT id, code FROM ledger.account WHERE code = @root_code
                UNION ALL
            ...
            WHERE je.voided_at IS NULL
              ...
              AND a.code IN (SELECT code FROM subtree)
              AND (
                (je.fiscal_period_id = @fiscal_period_id AND je.created_at <= @closed_at)
        ...
        cmd.Parameters.AddWithValue("@root_code", rootAccountCode) |> ignore
        cmd.Parameters.AddWithValue("@fiscal_period_id", fiscalPeriodId) |> ignore
        cmd.Parameters.AddWithValue("@closed_at", closedAt) |> ignore
        use reader = cmd.ExecuteReader()
        let mutable results = []
        while reader.Read() do
            let accountTypeName = reader.GetString(3)
